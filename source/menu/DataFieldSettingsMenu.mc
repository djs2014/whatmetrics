...
class DataFieldSettingsMenu extends WatchUi.Menu2 {
  function initialize() { Menu2.initialize({ : title => "Settings"}); }
}
...
class DataFieldSettingsMenuDelegate extends WatchUi.Menu2InputDelegate {
  ...
  function onSelect(menuItem as MenuItem) as Void {
    ...
    if (id instanceof String && id.equals("hiit")) {
      var hiitMenu =
          new WatchUi.Menu2({ : title => "High-intensity interval"});

      var mi = new WatchUi.MenuItem("Mode", null, "hiit_mode", null);
      var value = getStorageValue(mi.getId() as String, WhatHiitt.HiitDisabled)
                      as WhatHiitt.HiitMode;
      mi.setSubLabel($.getHiittModeText(value));
      hiitMenu.addItem(mi);

      mi = new WatchUi.MenuItem("Sound", null, "hiit_sound", null);
      value = getStorageValue(mi.getId() as String, WhatHiitt.NoSound)
                  as WhatHiitt.HiitSound;
      mi.setSubLabel($.getHiittSoundText(value));
      hiitMenu.addItem(mi);

      mi = new WatchUi.MenuItem("Start when % of target", null,
                                "hiit_startperc", null);
      mi.setSubLabel($.getStorageNumberAsString(mi.getId() as String));
      hiitMenu.addItem(mi);

      mi = new WatchUi.MenuItem("Stop when % of target", null, "hiit_stopperc",
                                null);
      mi.setSubLabel($.getStorageNumberAsString(mi.getId() as String));
      hiitMenu.addItem(mi);

      mi = new WatchUi.MenuItem("Sec countdown to start", null,
                                "hiit_countdown", null);
      mi.setSubLabel($.getStorageNumberAsString(mi.getId() as String));
      hiitMenu.addItem(mi);

      mi = new WatchUi.MenuItem("Sec inactivity until stop", null,
                                "hiit_inactivity", null);
      mi.setSubLabel($.getStorageNumberAsString(mi.getId() as String));
      hiitMenu.addItem(mi);

      WatchUi.pushView(hiitMenu, new $.HiittMenuDelegate(self, hiitMenu),
                       WatchUi.SLIDE_UP);
      ...
      WatchUi.pushView(targetMenu, new $.TargetsMenuDelegate(self, targetMenu),
                       WatchUi.SLIDE_UP);
...
class HiittMenuDelegate extends WatchUi.Menu2InputDelegate {
  ...
  function initialize(delegate as DataFieldSettingsMenuDelegate,
  ...
  function onSelect(item as MenuItem) as Void {
    ...
    if (id instanceof String && id.equals("hiit_mode")) {
      ...
      Toybox.WatchUi.pushView(view, new $.NumericInputDelegate(_debug, view),
                              WatchUi.SLIDE_RIGHT);
    }
  }
  ...
  function onNumericinput(editData as Array<Char>, cursorPos as Number,
                          insert as Boolean) as Void {
  ...
  function onDone() as Void { WatchUi.popView(WatchUi.SLIDE_DOWN); }
}
